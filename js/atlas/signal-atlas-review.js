...
if (new URLSearchParams(location.search).has('review')) {
  ...
  let videoURL = null;
  const state = () => window.signalAtlas.getState();
  ...
  document.getElementById('reviewRenderBtn').onclick = () => run(async () => {
    await window.signalAtlas.renderAt(Number(document.getElementById('seekSeconds').value) || 0);
    report(state());
  });
  ...
  document.getElementById('reviewFixturesBtn').onclick = () => run(async () => report(await runFixtureChecks(window.signalAtlas)));
  document.getElementById('reviewMixControlsBtn').onclick = () => run(async () => report(await runMixControlChecks(window.signalAtlas)));
  document.getElementById('reviewRecordBtn').onclick = () => run(async () => {
    await window.signalAtlas.pause();
    const before = state();
    const chunks = [];
    let closed = false;
    await window.signalAtlas.recordClip({ duration: 1, writable: {
    ...
    const checks = { wroteVideo: blob.size > 1000, closed, stopped: !after.recording,
      ...
      unlocked: !after.busy, remainedPaused: !after.playing };
  ...
  document.getElementById('reviewCancelBtn').onclick = () => run(async () => {
    ...
    let result;
    try { result = await window.signalAtlas.exportClip({ start: before.position, duration: 10, writable }); }
  ...
  document.getElementById('reviewStabilityBtn').onclick = () => run(async () => {
    ...
    try {
      await window.signalAtlas.exportClip({ start: 80, duration: 30, writable: {
  ...
  document.getElementById('reviewClipBtn').onclick = () => run(async () => {
    ...
    await window.signalAtlas.exportClip({ start, duration: 2, writable });
    ...
    output.innerHTML = `<button id="reviewClose" class="action glass">Close encoded preview</button>
      ...
      <a id="reviewDownload" download="signal-atlas-review.mp4">Download encoded clip</a>
    ...
    video.onseeked = async () => {
      ...
      await window.signalAtlas.renderAt(absoluteTrackTime);
      const encoded = ctx.getImageData(0, 0, sample.width, Math.floor(sample.height * .75)).data;
      ctx.drawImage(document.querySelector('#stage canvas'), 0, 0, sample.width, sample.height);
      const preview = ctx.getImageData(0, 0, sample.width, Math.floor(sample.height * .75)).data;
      let difference = 0;
      for (let i = 0; i < encoded.length; i += 4) {
        difference += Math.abs(encoded[i] - preview[i]) + Math.abs(encoded[i + 1] - preview[i + 1]) + Math.abs(encoded[i + 2] - preview[i + 2]);
      }
      document.getElementById('encodedInfo').textContent = JSON.stringify({ width: video.videoWidth, height: video.videoHeight, duration: video.duration, bytes: total, absoluteTrackTime, bottomQuarterMaxRGB: max,
