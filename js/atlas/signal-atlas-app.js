...
try {
  ...
  $('fatal').textContent = 'Signal Atlas could not load its Inter font. Reload the page to try again.';
  $('statusText').textContent = 'Font unavailable · ' + error.message;
  throw error;
}
let scene;
try {
  scene = createSignalAtlasScene(stage, settings);
} catch (error) {
  if ($('fatal')) {
    $('fatal').hidden = false;
    $('fatal').style.display = 'flex';
    $('fatal').textContent = 'Signal Atlas needs WebGL 2 and a GPU with 16,384-pixel textures. ' + error.message;
  }
...
function resizePreview() {
  ...
  resize(rect.width || innerWidth, rect.height || innerHeight, devicePixelRatio || 1);
}
...
async function seek(seconds, { resume = audio.isPlaying } = {}) {
  ...
  updateButtons();
  setStatus('Restoring 24 seconds of audio detail…', false);
  try {
...
async function exportVideo({ start = 0, duration = audio.duration, writable = null, clip = false } = {}) {
  ...
  try {
    const result = await capture.runOfflineExport({
      audio: exportAudio,
      suggestedName: clip ? 'signal-atlas-excerpt.mp4' : 'signal-atlas.mp4',
      pendingWritable: writable,
      onStatus: setStatus,
      labels: { rendering: 'Rendering Signal Atlas at 4K…', progress: 'Rendering frame {frame} of {total}', saved: 'MP4 saved · 1920 × 1080 · 60 fps' },
      async prepare() {
...
window.signalAtlas = Object.freeze({
  loadReference,
  ...
  async renderAt(seconds) {
    ...
    return result ? window.signalAtlas.getState() : null;
  },
